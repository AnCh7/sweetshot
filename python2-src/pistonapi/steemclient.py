...
class ExampleConfig(object):
    u""" The behavior of your program (e.g. reactions on messages) can be
    ...
    wallet_host = u"localhost"
    wallet_port = 8092
    wallet_user = u""
    wallet_password = u""

    #: Witness connection parameter
    witness_url = u"ws://localhost:8090/"
    witness_user = u""
    witness_password = u""


class SteemClient(object):
    u""" The ``SteemClient`` class is an abstraction layer that makes the use of the
    ...
    def __init__(self, config, **kwargs):
        u""" Initialize configuration
        """
        available_features = dir(config)

        if (u"wallet_host" in available_features and
                u"wallet_port" in available_features):
            self.wallet_host = config.wallet_host
            self.wallet_port = config.wallet_port

            if (u"wallet_user" in available_features and
                    u"wallet_password" in available_features):
        ...
        if u"witness_url" in available_features:
            self.witness_url = config.witness_url

            if (u"witness_user" in available_features):
                self.witness_user = config.witness_user

            if (u"witness_password" in available_features):
