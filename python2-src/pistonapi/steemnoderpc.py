...
import logging
import warnings
warnings.filterwarnings(u'default', module=__name__)
...
class SteemNodeRPC(GrapheneWebsocketRPC):
    ...
    def __init__(self,
        ...
        self.apis = kwargs.pop(
            u"apis",
            [u"database", u"network_broadcast"]
        )
    ...
    def register_apis(self, apis=None):
        for api in (apis or self.apis):
            api = api.replace(u"_api", u"")
            self.api_id[api] = self.get_api_by_name(u"%s_api" % api, api_id=1)
            if not self.api_id[api] and not isinstance(self.api_id[api], int):
                raise NoAccessApi(u"No permission to access %s API. " % api)
    ...
    def account_history(self, account, first=99999999999,
        ...
        warnings.warn(
            u"The block_stream() call has been moved to `steem.account.Account.rawhistory()`",
            DeprecationWarning
        )
    ...
    def block_stream(self, start=None, stop=None, mode=u"irreversible"):
        warnings.warn(
            u"The block_stream() call has been moved to `steem.blockchain.Blockchain.blocks()`",
            DeprecationWarning
        )
    ...
    def stream(self, opNames, *args, **kwargs):
        warnings.warn(
            u"The stream() call has been moved to `steem.blockchain.Blockchain.stream()`",
            DeprecationWarning
        )
        from piston.blockchain import Blockchain
        return Blockchain(mode=kwargs.get(u"mode", u"irreversible")).stream(opNames, *args, **kwargs)

    def list_accounts(self, start=None, step=1000, limit=None, **kwargs):
        warnings.warn(
            u"The list_accounts() call has been moved to `steem.blockchain.Blockchain.get_all_accounts()`",
            DeprecationWarning
        )
        from piston.blockchain import Blockchain
        return Blockchain(mode=kwargs.get(u"mode", u"irreversible")).get_all_accounts(start=start, steps=step, **kwargs)

    def get_network(self):
        u""" Identify the connected network. This call returns a
            dictionary with keys chain_id, prefix, and other chain
            specific settings
        """
        props = self.get_dynamic_global_properties()
        chain = props[u"current_supply"].split(u" ")[1]
        assert chain in known_chains, u"The chain you are connecting to is not supported"
    ...
    def rpcexec(self, payload):
        ...
        try:
            ...
            if msg == u"Account already transacted this block.":
                raise exceptions.AlreadyTransactedThisBlock(msg)
            elif msg == u"missing required posting authority":
                raise exceptions.MissingRequiredPostingAuthority
            elif msg == u"Voting weight is too small, please accumulate more voting power or steem power.":
                raise exceptions.VoteWeightTooSmall(msg)
            elif msg == u"Can only vote once every 3 seconds.":
                raise exceptions.OnlyVoteOnceEvery3Seconds(msg)
            elif msg == u"You have already voted in a similar way.":
                raise exceptions.AlreadyVotedSimilarily(msg)
            elif msg == u"You may only post once every 5 minutes.":
                raise exceptions.PostOnlyEvery5Min(msg)
            elif msg == u"Duplicate transaction check failed":
                raise exceptions.DuplicateTransaction(msg)
            elif msg == u"Account exceeded maximum allowed bandwidth per vesting share.":
                raise exceptions.ExceededAllowedBandwidth(msg)
            elif re.match(u"^no method with name.*", msg):
