...
class Converter(object):
    u""" Converter simplifies the handling of different metrics of
        the blockchain
    ...
    def sbd_median_price(self):
        u""" Obtain the sbd price as derived from the median over all
            witness feeds. Return value will be SBD
        """
        return (
            Amount(self.steem.rpc.get_feed_history()[u'current_median_history'][u'base']).amount /
            Amount(self.steem.rpc.get_feed_history()[u'current_median_history'][u'quote']).amount
        )

    def steem_per_mvests(self):
        u""" Obtain STEEM/MVESTS ratio
        """
    ...
    def vests_to_sp(self, vests):
        u""" Obtain SP from VESTS (not MVESTS!)
    ...
    def sp_to_vests(self, sp):
        u""" Obtain VESTS (not MVESTS!) from SP
    ...
    def steem_to_sbd(self, amount_steem):
        u""" Conversion Ratio for given amount of STEEM to SBD at current
            price feed
    ...
    def sbd_to_steem(self, amount_sbd):
        u""" Conversion Ratio for given amount of SBD to STEEM at current
            price feed
    ...
    def sbd_to_rshares(self, sbd_payout):
        ...
        total_reward_fund_steem = Amount(props[u'total_reward_fund_steem'])
        total_reward_shares2 = int(props[u'total_reward_shares2'])
