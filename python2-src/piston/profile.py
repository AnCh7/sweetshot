...
class DotDict(dict):
    ...
    def __init__(self, *args):
        u""" This class simplifies the use of "."-separated
...
class Profile(DotDict):
    u""" This class is a template to model a user's on-chain
    ...
    def remove(self, key):
        parts = key.split(u".")
        if len(parts) > 1:
            self[parts[0]].pop(u".".join(parts[1:]))
        else:
            super(Profile, self).pop(parts[0], None)


if __name__ == u'__main__':
    keys = [u'profile.url', u'profile.img']
    values = [u"http:", u"foobar"]
    print Profile(keys, values)
    print Profile({u"foo": u"bar"})
