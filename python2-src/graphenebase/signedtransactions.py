...
try:
    import secp256k1
    USE_SECP256K1 = True
    log.debug(u"Loaded secp256k1 binding.")
except:
    USE_SECP256K1 = False
    log.debug(u"To speed up transactions signing install \n"
              u"    pip install secp256k1")


class Signed_Transaction(GrapheneObject):
    u""" Create a signed transaction and offer method to create the
        signature
    ...
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            ...
            if len(args) == 1 and len(kwargs) == 0:
                kwargs = args[0]
            if u"extensions" not in kwargs:
                kwargs[u"extensions"] = Set([])
            elif not kwargs.get(u"extensions"):
                kwargs[u"extensions"] = Set([])
            if u"signatures" not in kwargs:
                kwargs[u"signatures"] = Array([])
            else:
                kwargs[u"signatures"] = Array([Signature(unhexlify(a)) for a in kwargs[u"signatures"]])

            if u"operations" in kwargs:
                opklass = self.getOperationKlass()
                if all([not isinstance(a, opklass) for a in kwargs[u"operations"]]):
                    kwargs[u'operations'] = Array([opklass(a) for a in kwargs[u"operations"]])
                else:
                    kwargs[u'operations'] = Array(kwargs[u"operations"])

            super(Signed_Transaction, self).__init__(OrderedDict([
                (u'ref_block_num', Uint16(kwargs[u'ref_block_num'])),
                (u'ref_block_prefix', Uint32(kwargs[u'ref_block_prefix'])),
                (u'expiration', PointInTime(kwargs[u'expiration'])),
                (u'operations', kwargs[u'operations']),
                (u'extensions', kwargs[u'extensions']),
                (u'signatures', kwargs[u'signatures']),
            ]))
    ...
    def recoverPubkeyParameter(self, digest, signature, pubkey):
        u""" Use to derive a number that allows to easily recover the
    ...
    def derSigToHexSig(self, s):
        u""" Format DER to HEX signature
        """
        s, junk = ecdsa.der.remove_sequence(unhexlify(s))
        if junk:
            log.debug(u'JUNK: %s', hexlify(junk).decode(u'ascii'))
        assert(junk == '')
        x, s = ecdsa.der.remove_integer(s)
        y, s = ecdsa.der.remove_integer(s)
        return u'%064x%064x' % (x, y)

    def compressedPubkey(self, pk):
        order = pk.curve.generator.order()
        p = pk.pubkey.point
        x_str = ecdsa.util.number_to_string(p.x(), order)
        returnstr(chr(2 + (p.y() & 1))).encode('ascii') + x_str

    def recover_public_key(self, digest, signature, i):
        u""" Recover the public key from the the signature
        """
    ...
    def getChainParams(self, chain):
        ...
        if isinstance(chain, unicode) and chain in chains:
            chain_params = chains[chain]
        elif isinstance(chain, dict):
            chain_params = chain
        else:
            raise Exception(u"sign() only takes a string or a dict as chain!")
        if u"chain_id" not in chain_params:
            raise Exception(u"sign() needs a 'chain_id' in chain params!")
        return chain_params

    def deriveDigest(self, chain):
        chain_params = self.getChainParams(chain)
        # Chain ID
        self.chainid = chain_params[u"chain_id"]

        # Do not serialize signatures
        sigs = self.data[u"signatures"]
        self.data[u"signatures"] = []

        # Get message to sign
        #   bytes(self) will give the wire formated data according to
        #   GrapheneObject and the data given in __init__()
        self.message = unhexlify(self.chainid) + str(self)
        self.digest = hashlib.sha256(self.message).digest()

        # restore signatures
        self.data[u"signatures"] = sigs
    ...
    def verify(self, pubkeys=[], chain=None):
        ...
        signatures = self.data[u"signatures"].data
        pubKeysFound = []

        for signature in signatures:
            sig = str(signature)[1:]
            recoverParameter = (str(signature)[0]) - 4 - 27  # recover parameter only
            ...
            if USE_SECP256K1:
                ...
                # Recover PublicKey
                verifyPub = secp256k1.PublicKey(pub.ecdsa_recover(str(self.message), sig))
                # Convert recoverable sig to normal sig
                normalSig = verifyPub.ecdsa_recoverable_convert(sig)
                # Verify
                verifyPub.ecdsa_verify(str(self.message), normalSig)
                phex = hexlify(verifyPub.serialize(compressed=True)).decode(u'ascii')
                ...
                phex = hexlify(self.compressedPubkey(p)).decode(u'ascii')
                pubKeysFound.append(phex)

        for pubkey in pubkeys:
            if not isinstance(pubkey, PublicKey):
                raise Exception(u"Pubkeys must be array of 'PublicKey'")

            k = pubkey.unCompressed()[2:]
            if k not in pubKeysFound and repr(pubkey) not in pubKeysFound:
                k = PublicKey(PublicKey(k).compressed())
                f = format(k, chain_params[u"prefix"])
                raise Exception(u"Signature for %s missing!" % f)
        return pubKeysFound
    ...
    def sign(self, wifkeys, chain=None):
        u""" Sign the transaction with the provided private keys.
        ...
        """
        if not chain:
            raise Exception(u"Chain needs to be provided!")
        self.deriveDigest(chain)
        ...
        for wif in self.privkeys:
            p = str(PrivateKey(wif))
            i = 0
            if USE_SECP256K1:
                ndata = secp256k1.ffi.new(u"const int *ndata")
                ndata[0] = 0
                while True:
                    ndata[0] += 1
                    privkey = secp256k1.PrivateKey(p, raw=True)
                    sig = secp256k1.ffi.new(u'secp256k1_ecdsa_recoverable_signature *')
                ...
                while 1:
                    cnt += 1
                    if not cnt % 20:
                        log.info(u"Still searching for a canonical signature. Tried %d times already!" % cnt)
                    ...
                    k = ecdsa.rfc6979.generate_k(
                        ...
                        hashlib.sha256(
                            self.digest +
                            struct.pack(u"d", time.time())  # use the local time to randomize the signature
                        ).digest())
            ...
            # pack signature
            #
            sigstr = struct.pack(u"<B", i)
            sigstr += signature

            sigs.append(Signature(sigstr))

        self.data[u"signatures"] = Array(sigs)
        return self
