...
class PasswordKey(object):
    u""" This class derives a private key given the account name, the
    ...
    def __init__(self, account, password, role=u"active"):
        self.account = account
        self.role = role
        self.password = password

    def get_private(self):
        u""" Derive private key from the brain key and the current sequence
            number
        """
        a =str(self.account +
                  self.role +
        ...
        return PrivateKey(hexlify(s).decode(u'ascii'))
...
class BrainKey(object):
    u"""Brainkey implementation similar to the graphene-ui web-wallet.
    ...
    def next(self):
        u""" Get the next private key (sequence number increment) for
            iterators
        """
        return self.next_sequence()

    def next_sequence(self):
        u""" Increment the sequence number by 1 """
        self.sequence += 1
        return self

    def normalize(self, brainkey):
        u""" Correct formating with single whitespace syntax and no trailing space """
        return u" ".join(re.compile(u"[\t\n\v\f\r ]+").split(brainkey))

    def get_brainkey(self):
        u""" Return brain key of this instance """
    ...
    def get_private(self):
        u""" Derive private key from the brain key and the current sequence
            number
        """
        encoded = u"%s %d" % (self.brainkey, self.sequence)
        a =str(encoded).encode('ascii')
    ...
    def suggest(self):
        u""" Suggest a new random brain key. Randomness is provided by the
            operating system using ``os.urandom()``.
        """
        word_count = 16
        brainkey = [None] * word_count
        dict_lines = BrainKeyDictionary.split(u',')
        assert len(dict_lines) == 49744
        for j in xrange(0, word_count):
            num = int.from_bytes(os.urandom(2), byteorder=u"little")
            rndMult = num / 2 ** 16  # returns float between 0..1 (inclusive)
            wIdx = round(len(dict_lines) * rndMult)
            brainkey[j] = dict_lines[wIdx]
        return u" ".join(brainkey).upper()
...
class Address(object):
    ...
    """
    def __init__(self, address=None, pubkey=None, prefix=u"GPH"):
        self.prefix = prefix
        if pubkey is not None:
            ...
            raise Exception(u"Address has to be initialized by either the " +
                            u"pubkey or the address.")

    def derivesha256address(self):
        u""" Derive address using ``RIPEMD160(SHA256(x))`` """
        pkbin = unhexlify(repr(self._pubkey))
        addressbin = ripemd160(hexlify(hashlib.sha256(pkbin).digest()))
        return Base58(hexlify(addressbin).decode(u'ascii'))

    def derivesha512address(self):
        u""" Derive address using ``RIPEMD160(SHA512(x))`` """
        pkbin = unhexlify(repr(self._pubkey))
        addressbin = ripemd160(hexlify(hashlib.sha512(pkbin).digest()))
        return Base58(hexlify(addressbin).decode(u'ascii'))

    def __repr__(self):
        u""" Gives the hex representation of the ``GrapheneBase58CheckEncoded``
            Graphene address.
        """
        return repr(self.derivesha512address())

    def __str__(self):
        u""" Returns the readable Graphene address. This call is equivalent to
            ``format(Address, "GPH")``
        """
        return format(self, self.prefix)

    def __format__(self, _format):
        u"""  May be issued to get valid "MUSE", "PLAY" or any other Graphene compatible
            address with corresponding prefix.
        """
        if self._address is None:
            if _format.lower() == u"btc":
    ...
    def __bytes__(self):
        u""" Returns the raw content of the ``Base58CheckEncoded`` address """
        if self._address is None:
            return str(self.derivesha512address())
        else:
            return str(self._address)


class PublicKey(Address):
    u""" This class deals with Public Keys and inherits ``Address``.
    ...
    """
    def __init__(self, pk, prefix=u"GPH"):
        self.prefix = prefix
    ...
    def compressed(self):
        u""" Derive compressed public key """
        order = ecdsa.SECP256k1.generator.order()
        p = ecdsa.VerifyingKey.from_string(str(self), curve=ecdsa.SECP256k1).pubkey.point
        x_str = ecdsa.util.number_to_string(p.x(), order)
        # y_str = ecdsa.util.number_to_string(p.y(), order)
        compressed = hexlify(str(chr(2 + (p.y() & 1))).encode('ascii') + x_str).decode(u'ascii')
        return(compressed)

    def unCompressed(self):
        u""" Derive uncompressed key """
        public_key = repr(self._pk)
        prefix = public_key[0:2]
        if prefix == u"04":
            return public_key
        assert prefix == u"02" or prefix == u"03"
        x = int(public_key[2:], 16)
        y = self._derive_y_from_x(x, (prefix == u"02"))
        key = u'04' + u'%064x' % x + u'%064x' % y
        return key

    def point(self):
        u""" Return the point for the public key """
        string = unhexlify(self.unCompressed())
        return ecdsa.VerifyingKey.from_string(string[1:], curve=ecdsa.SECP256k1).pubkey.point

    def __repr__(self):
        u""" Gives the hex representation of the Graphene public key. """
        return repr(self._pk)

    def __str__(self):
        u""" Returns the readable Graphene public key. This call is equivalent to
            ``format(PublicKey, "GPH")``
        """
        return format(self._pk, self.prefix)

    def __format__(self, _format):
        u""" Formats the instance of:doc:`Base58 <base58>` according to ``_format`` """
        return format(self._pk, _format)

    def __bytes__(self):
        u""" Returns the raw public key (has length 33)"""
        return str(self._pk)


class PrivateKey(PublicKey):
    u""" Derives the compressed and uncompressed public keys and
    ...
    """
    def __init__(self, wif=None, prefix=u"GPH"):
        if wif is None:
            import os
            self._wif = Base58(hexlify(os.urandom(32)).decode(u'ascii'))
    ...
    def compressedpubkey(self):
        u""" Derive uncompressed public key """
        ...
        y_str = ecdsa.util.number_to_string(p.y(), order)
        compressed = hexlify(str(chr(2 + (p.y() & 1))).encode('ascii') + x_str).decode(u'ascii')
        uncompressed = hexlify(str(chr(4)).encode('ascii') + x_str + y_str).decode(u'ascii')
        return([compressed, uncompressed])

    def __format__(self, _format):
        u""" Formats the instance of:doc:`Base58 <base58>` according to
            ``_format``
        """
        return format(self._wif, _format)

    def __repr__(self):
        u""" Gives the hex representation of the Graphene private key."""
        return repr(self._wif)

    def __str__(self):
        u""" Returns the readable (uncompressed wif format) Graphene private key. This
            call is equivalent to ``format(PrivateKey, "WIF")``
        """
        return format(self._wif, u"WIF")

    def __bytes__(self):
        u""" Returns the raw private key """
